...
FinalCbTask.prototype.exec = function (err) {
  ...
  if (err) {
    this.execOptions.flowEmitter.emit(EventManager.TYPES.EXEC_FLOW_ERRORED, this.execOptions.flowEnv);
    this.f.call(null, err); //call the final callback with the first error hit
  } else { // no error, call with args
    this.execOptions.flowEmitter.emit(EventManager.TYPES.EXEC_FLOW_COMPLETE, this.execOptions.flowEnv);
    var vCon = this.vCon;
