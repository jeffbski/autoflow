    ...
    function execTask(t){
      ...
      try {
        ...
        if (typeof(t.f) === 'string') { //object method call
          var objAndMeth = t.f.split('.');
          var objName = objAndMeth.shift();
          var methName = objAndMeth.shift();
          var obj = vCon[objName];
          ret = obj[methName].apply(obj, args); //exec method
        } else { //function call
          ret = t.f.apply(null, args);          //exec function
        }  
