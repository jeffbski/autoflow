...
/**
   @example
   var r = react('filename, uid').define(
...
function formatErrorMeta(err){
  ...
  return '\n\n' +
      'Error occurs in Task function: ' + task.f.name + '(' + task.a.join(',') + ')\n\n' + 
      'Variable Context: \n' +
      util.inspect(vcon) + '\n\n' +
      'Task Source:\n\n' +
...
function nameTasks(tasks){ //name tasks that are not already named, validation done elsewhere, ret map
  ...
  tasks.forEach(function(t, idx){
    if (!t.name) { //not already named
      var name = t.f.name; //try to use function name
...
function parseTaskDefs(arrArgs){ // [fun, strArgsCbArgs, optObj]
  ...
  while(arrArgs.length){
    ...
    if (typeof(fn) !== 'function') throw "error parsing taskdef, expected function, got:"+fn;
    ...
    if (taskDef.after) {
        ...
        taskDef.after = taskDef.after.map(function(a){ return (a.name) ? a.name : a; }); //if fn, use name
    }
  ...
  function exec(arg1, arg2, argN){
    ...
    function createCallback(task) {
      return function(err, arg0, arg1, argn){
        ...
        if (reactOptions.debugOutput) console.log('in callback: %s cb:', task.f.name, args, vCon);
        contExec();
      };
    }
    ...
    contExec = function contExec(){
      ...
      tasksReady.forEach(function(t){
        ...
        if (reactOptions.debugOutput) console.log('starting task: %s', t.f.name, args);
        try { var ret = t.f.apply(null, args);    //exec function
              if (t.ret) { vCon[t.ret] = ret;     //if non-cb fn, save retval
