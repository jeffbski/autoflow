...
var REQ = 'retTask requires f, in, out';
var FN_REQ = 'retTask requires f to be a function or string';
var A_REQ = 'retTask requires in to be an array of string param names';
...
RetTask.validate = function (taskDef) {
  var errors = [];
  if (!taskDef.f || !taskDef.in || !taskDef.out) {
    ...
    if (! (Array.isArray(taskDef.in) &&
           taskDef.in.every(function (x) { return (typeof(x) === 'string'); }))) {
...
RetTask.prototype.exec = function exec(vCon, handleError, contExec) {
  try {
    var args = this.in.map(function (k) { return vCon.getVar(k); }); //get args from vCon
    var func = this.f;
