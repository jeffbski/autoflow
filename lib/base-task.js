...
BaseTask.prototype.complete = function (args) { //args that were used are available
  if (this.flowEmitter) this.flowEmitter.emit(rEvents.EVENTS.TASK_COMPLETE, this.name, args, this);
...
BaseTask.prototype.depTasksAreDone = function (tasksByName) {
  var afterArr = array(this.after);  // ensure is array, null becomes []
  return (!afterArr.length || // no dep tasks OR
          afterArr.every(function (n) { return tasksByName[n].isComplete(); }));  //all done
};
