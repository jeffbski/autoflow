...
define(['./eventemitter', './error', './validate', './task', './status',
  ...
  */
  function reactFactory() {
    ...
    function createSubflows() { // create subflows if any, return array of errors
      return Object.keys(ast.sub).reduce(function (errors, subName) {
        var subReact = reactFactory();
        var subErrors = subReact.setAndValidateAST(ast.sub[subName]);
        ast.sub[subName] = subReact;
    ...
    function setAndValidateAST(newAST) { //set AST then validate, ret error[]
      ...
      errors = errors.concat(createSubflows());
      return errors;
    }
