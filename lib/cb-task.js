...
/**
   If callback params are obj property writes make sure the dst objects
   are defined and not null. cb: ['b.c'] -> b is def and not null
   @returns true if all obj prop parents are def and non null
  */
CbTask.prototype.outParentsExist = function (vCon) {
  var self = this;
  return this.cb.every(function (x) { return self.parentExists(x, vCon) });
};
