...
function execTasks(tasksReady, vCon, handleError) {
  tasksReady.forEach(function (t) { t.status = STATUS.READY; }); //set ready first, no double exec
  tasksReady.forEach(function (t) { t.exec(vCon, handleError); }); 
}

function findReadyAndExec(vCon, tasks, tasksByName, handleError) {
  var tasksReady = findTasksReady(vCon, tasks, tasksByName);
  execTasks(tasksReady, vCon, handleError);
}
