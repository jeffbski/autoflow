...
function validate(ast) {
  ...
  if (errors.length === 0) { // if no errors do additional validation
    errors = errors.concat(validateOuputsUnique(ast.tasks));
  }
  return errors;
}
...
function getOutputParams(taskDef) {
  var opk = tutil.TASK_TYPES[taskDef.type].outputParamsKey;
  var outputParams = taskDef[opk];
  return array(outputParams); //ensure array
}
