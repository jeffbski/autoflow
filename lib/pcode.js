...
function pcode(inParamStr, taskStrArr, locals) {
  ...
  var errors = reactFn.setAndValidateAST({
    ...
    outTask: { in: parseOutParams(outTaskStr).outDef },
    locals: locals
  });
  if (errors.length) {
...
function selectFirst(inParamStr, taskStrArr, locals) {
  if (!inParamStr) inParamStr = '';
  if (!taskStrArr) taskStrArr = [];
  var outTaskStr = (taskStrArr.length) ? taskStrArr.pop() : '';
  if (!locals) locals = { };
  ...
  var errors = reactFn.setAndValidateAST({
    inParams: parseInParams(inParamStr).inDef,
    tasks: tasks,
    outTask: { type: 'finalcbFirst', in: parseOutParams(outTaskStr).outDef },
    locals: locals
  });
  if (errors.length) {
    var errorStr = errors.join('\n');
