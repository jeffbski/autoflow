...
function splitArgs(args, inParams, style) {
  ...
  if (style === OUTPUT_STYLES.CALLBACK) {
    if (typeof(args[0]) === 'function') result.cb = args.shift(); // next take the cb
    else { // no callback provided, switch to promise style
      result.outputStyle = OUTPUT_STYLES.PROMISE;
    }
  }
