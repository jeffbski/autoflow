...
/**
   exports:
    - react - React's factory method 
    - reactOptions - options object for configuring React
...
var tCommon = trait({
  /* Optional items
     status, a, after, tasksByName
   */
  vCon:        trait.required,
  isFirstError: trait.required,
  setFirstError: trait.required,
  getFinalCallback: trait.required,
  augmentError: function() { /* TODO */ },
  statusEnum: { READY: 'ready',  RUNNING: 'running', ERRORED: 'errored', COMPLETE: 'complete' },
  getVar: function (name) { //get the name from the variable context, name might be simple or obj.prop
      var nameAndProps = name.split('.');
      return nameAndProps.reduce(function(accObj, prop){ return accObj[prop]; }, this.vCon); // vCon['foo']['bar']
  },
  stripPropertyName: function(objectWithOptionalPropertyName) { //return only object if object.property
    return ( /\w+/ ).exec(objectWithOptionalPropertyName)[0];
  },
  isReady: function() {
    return !this.status &&                                                // filter for not started AND
           this.a.every(function(k){
             return (this.vCon[this.stripPropertyName(k)] !== undefined); }) &&   // all dep vars defined AND
             (!this.after ||                                              // (no dep tasks OR 
               this.after.every( function(n){
                 return this.tasksByName[n].status === this.statusEnum.COMPLETE; })); //alldone
  },
  setStatusRunning: function (){ this.status = this.statusEnum.RUNNING; },
  handleTaskError: function (task, err){
      this.status = STATUS.ERRORED;
      if (!this.isFirstError()) { //no prev error, only calling final callback with error once
        var errWithMeta = this.augmentError(err, {task:task, vcon:vCon});
        this.setFirstError(errWithMeta); //save this, stop other tasks from being launched
        this.getFinalCallback().call(null, errWithMeta); //call the final callback with the first error hit
      }
    }

});

var tFunctionWithCB = trait({
  f:                trait.required,
  a:                trait.required,
  cbFun:            trait.required,
  getVar:           trait.required,
  handleTaskError:  trait.required,
  isReady:          trait.required,
  setStatusRunning: trait.required,
  exec: function() {
    this.setStatusRunning();
    var args = this.a.map(function(k){ return this.getVar(k); }); //get args from vCon
    args.push(this.cbFun); 
    try { this.f.apply(null, args); }
    catch (err) { this.handleTaskError(this, err); }    //catch and handle the task error, calling final cb
  }   
});

var tFunctionWithReturn = trait({
  f: trait.required,
  a: trait.required,
  ret: trait.required,
  vCon: trait.required,
  getVar: trait.required,
  statusEnum: trait.required,
  handleTaskError: trait.required,
  contExec: trait.required,
  isReady: trait.required,
  setStatusRunning: trait.required,
  exec: function() {
    this.setStatusRunning();
    var args = this.a.map(function(k){ return this.getVar(k); }); //get args from vCon
    try {
      this.vCon[this.ret] = this.f.apply(null, args);  //  save retval
      this.status = this.statusEnum.COMPLETE;          //  mark complete
      this.contExec();                                 //  continue since no callback to run this
    } catch (err) { this.handleTaskError(this, err); }    //catch and handle the task error, calling final cb
  }   
});
  

var tFunctionWithCBTask = trait.compose(
  tCommon,
  tFunctionWithCB
);

var tFunctionWithReturnTask = trait.compose(
  tCommon,
  tFunctionWithCB
);


function taskFactory(flowContext) {
  return tFunctionWithReturnTask.create({
    vCon: flowContext.vCon,
    f: function () { /* todo */ },
    a: [],
    cbFun: function () { /* todo */ },
    isFirstError: function () { return (flowContext.firstError); },
    setFirstError: function (err) { flowContext.firstError = err; },
    getFinalCallback: function () { return flowContext.finalCallback; }
  });
}
...
var foo = taskFactory(flowContext);
