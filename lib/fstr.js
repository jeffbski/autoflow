...
function fstr(inParamStr, taskDefArr, outParamStr, locals) {
  ...
  var errors = reactFn.setAndValidateAST({
    ...
    outTask: { in: parseOutParams(outParamStr).outDef },
    locals: locals
  });
  if (errors.length) {
...
function selectFirst(inParamStr, taskDefArr, outParamStr, locals) {
  if (!inParamStr) inParamStr = '';
  if (!taskDefArr) taskDefArr = [];
  if (!outParamStr) outParamStr = '';
  if (!locals) locals = {};
  ...
  var errors = reactFn.setAndValidateAST({
    inParams: parseInParams(inParamStr).inDef,
    tasks: tasks,
    outTask: { type: 'finalcbFirst', in: parseOutParams(outParamStr).outDef },
    locals: locals
  });
  if (errors.length) {
    var errorStr = errors.join('\n');
